...
performance to other parcellations (when evaluated with the same [ML Pipeline](./ml_pipelines.html) and for the same [target variable](./variables.html)).
For example consider the highly simplified example below:
...
A key benefit of Mean Rank over employing metrics like
