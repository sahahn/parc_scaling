...
def surf_to_surf(in_loc, hemi, cmd_type='-metric-resample'):
    ...
    std_mesh_dr = os.path.abspath('../raw/standard_mesh_atlases/')
...
def annot_to_gifti(lh_loc, rh_loc):
    ...
    lh_w = os.path.join(freesurfer_loc, 'subjects', 'fsaverage/surf/lh.white')
    rh_w = os.path.join(freesurfer_loc, 'subjects', 'fsaverage/surf/rh.white')
    ...
    with open('temp.sh', 'w') as f:
        f.write(fs_cmd + ' && ')
        f.write('mris_convert --annot ' + lh_loc + ' ' + lh_w + ' ' + lh_gifti + ' && ')
        f.write('mris_convert --annot ' + rh_loc + ' ' + rh_w + ' ' + rh_gifti)
...
def prob_vol_labels_to_surf(vol):
    ...
    for i in range(data.shape[-1]):
        ...
        # If there is an error for converting just
        # one map, then skip it.
        except:
            continue
...
def fsaverage_label_to_fs_lr(lh_loc, rh_loc):

    # Convert to gifti (don't delete in place)
    lh_gifti, rh_gifti = annot_to_gifti(lh_loc, rh_loc)

    # Convert to FS_lr_32k
    lh_loc = surf_to_surf(lh_gifti, 'L', '-label-resample')
    rh_loc = surf_to_surf(rh_gifti, 'R', '-label-resample')
